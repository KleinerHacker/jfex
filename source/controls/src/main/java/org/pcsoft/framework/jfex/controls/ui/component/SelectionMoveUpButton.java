...
public class SelectionMoveUpButton extends Button {
    ...
    public SelectionMoveUpButton() {
        super(null, new ImageView(new Image(SelectionMoveUpButton.class.getResourceAsStream("/icons/ic_up16.png"))));
    ...
    protected void updateDisabledStateBinding(Node n) {
        ...
        final BooleanBinding onTopDisabledState = Bindings.integerValueAt(selectedIndicesProperty, 0).lessThanOrEqualTo(shiftCount.subtract(1));
        ...
        disableProperty().bind(noSelectionDisabledState.or(multiSelectionDisabledState).or(onTopDisabledState).or(filterDisabledState));
    }

    public SelectionMoveUpButton(final Node forList) {
        this();
        ...
        if (selectionModel != null && items != null) {
            for (final int index : new ArrayList<>(selectionModel.getSelectedIndices())) {
                for (int i = 0; i < shiftCount.get(); i++) {
                    final Object elementTop = items.get(index - (i + 1));
                    final Object elementBottom = items.get(index - i);
                    items.removeAll(Arrays.asList(elementTop, elementBottom));
                    items.addAll(index - (i + 1), (List) Arrays.asList(elementBottom, elementTop));
                }
            }
        }
    }
