...
public class ComboBoxEx<T, G> extends StackPane implements SimpleDataModel<T, G, ListCell> {
    ...
    public ComboBoxEx() {
        final Fxml.FxmlTuple<? extends FxmlView, FxmlViewModel> viewTuple =
                Fxml.from(ComboBoxExView.class).withResources(ResourceBundle.getBundle("lan/text")).withRoot(this).load();
        controller = viewTuple.getViewController();

        viewModel = (ComboBoxExViewModel<T, G>) viewTuple.getViewModel();
        view = (ComboBoxExView) viewTuple.getViewController();
    }
